# Qt GUI Migration Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Migrate natt-gui from webview (HTML/CSS/JS) to Qt 6 Widgets (C++)

**Architecture:** Replace the webview window + embedded HTML approach with a QMainWindow-based Qt Widgets UI. CoreClient interface remains unchanged. Build dependencies switch from FetchContent webview to Conan qt/6.11.1.

**Tech Stack:** C++20, Qt 6.11.1 (Core, Widgets), Conan, CMake, natcore

---

### Task 1: Build system — add Qt dependency
...
- [ ] **Step 1: Add qt/6.11.1 to conanfile.txt**
...
- [ ] **Step 2: Replace webview FetchContent with Qt6 find_package in dependencies.cmake**
...
- [ ] **Step 3: Update CMakeLists.txt natt-gui target**

Edit `CMakeLists.txt` (lines 154-174):
```diff
 # ── natt-gui (optional, requires WebKitGTK) ─────────────────────
...
```bash
git add conanfile.txt cmake/dependencies.cmake CMakeLists.txt
git commit -m "build: replace webview with Qt 6.11.1 for natt-gui"
```

---

### Task 2: Rewrite main.cpp to Qt Widgets

**Files:**
- Create: `cmd/gui/main.cpp` (full rewrite)

- [ ] **Step 1: Write the complete Qt Widgets main.cpp**

File: `cmd/gui/main.cpp`
...
```bash
git add cmd/gui/main.cpp
git commit -m "feat: rewrite natt-gui with Qt Widgets"
```

---

### Task 3: Remove webview residuals

**Files:**
- Delete: `cmd/gui/index.html`

- [ ] **Step 1: Delete index.html**

```bash
git rm cmd/gui/index.html
```

- [ ] **Step 2: Remove the `include(FetchContent)` line if no longer needed in dependencies.cmake**

Check if `FetchContent` is still used anywhere else in `dependencies.cmake`. If not, remove the `include(FetchContent)` line.
...
git rm cmd/gui/index.html
git commit -m "chore: remove webview HTML and unused FetchContent"
```
...
- [ ] **Step 1: Install Qt 6.11.1 via Conan**
...
- [ ] **Step 3: Build natt-gui target**

```bash
cmake --build build --target natt-gui
```

- [ ] **Step 4: Verify other targets still build**

```bash
cmake --build build --target natt-cli
cmake --build build --target natmesh-server
```
...
```bash
git add -A
git commit -m "fix: build fixes after Qt migration"
```
