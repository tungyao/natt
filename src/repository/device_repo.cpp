...
bool DeviceRepo::update_virtual_ip(const std::string& node_id,
                                    const std::string& virtual_ip) {
    const char* sql = "UPDATE devices SET virtual_ip=?, updated_at=datetime('now') WHERE node_id=?;";
    sqlite3_stmt* stmt = nullptr;
    ...
    sqlite3_bind_text(stmt, 1, virtual_ip.c_str(), -1, SQLITE_TRANSIENT);
...
std::vector<Device> DeviceRepo::find_by_user_id(int64_t user_id) {
    const char* sql = "SELECT id, node_id, user_id, device_name, public_key, public_ip, public_port, lan_ips, virtual_ip, online, last_heartbeat, created_at, updated_at FROM devices WHERE user_id = ?;";
    sqlite3_stmt* stmt = nullptr;
...
std::vector<Device> DeviceRepo::find_offline_timeout(int timeout_sec) {
    const char* sql = R"(
        SELECT id, node_id, user_id, device_name, public_key, public_ip, public_port, lan_ips, virtual_ip, online, last_heartbeat, created_at, updated_at
